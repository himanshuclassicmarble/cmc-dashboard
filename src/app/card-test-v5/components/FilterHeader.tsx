...
<Popover open={open} onOpenChange={setOpen}>
  <PopoverTrigger asChild>
    <Button
      ...
      aria-expanded={open}
      className={cn(
        "h-6 px-2 text-xs rounded-sm border border-border bg-background hover:bg-accent hover:text-accent-foreground",
        selectedExecutive && " border-secondary text-secondary-foreground hover:bg-accent/70"
      )}
    >
      {/* Mobile Avatar */}
      <div className="flex items-center sm:hidden">
      ...
      <div className="hidden sm:flex items-center ">
        {selectedExecutive ? (
          <div className="flex items-center gap-1">
            ...
            <span className="truncate font-medium max-w-20 text-foreground">
              {selectedExecutive.name}
            </span>
          </div>
        ) : (
          <>
            ...
            <span className="truncate max-w-16 text-muted-foreground">Executives</span>
          </>
        )}
      </div>

      <ChevronDown className="ml-1 h-3 w-3 transition-transform duration-200 text-muted-foreground group-data-[state=open]:rotate-180" />
    </Button>
  </PopoverTrigger>
          ...
          >
            <div className="flex items-center gap-2 w-full">
              ...
              <div className="flex-1">
                <div className="text-xs font-medium text-foreground">All Executives</div>
                <div className="text-xs text-muted-foreground">Show all</div>
              </div>
              <Check className={cn("ml-auto h-3 w-3", selectedExecutive === null ? "opacity-100" : "opacity-0")} />
            </div>
          </CommandItem>
            ...
            >
              <div className="flex items-center gap-2 w-full">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-medium text-foreground truncate">{executive.name}</div>
